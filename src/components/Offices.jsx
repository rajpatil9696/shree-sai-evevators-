export default function Offices() {
  return (
    <section className="offices" id="offices">
      <div className="container">
        <h2 className="section-title">Reach us across Maharashtra</h2>
        <div className="offices-grid">
          <article className="office-card office-card--hq">
            <span className="office-card__tag">Head Office</span>
            <h3>Baramati</h3>
            <p>Shop no 18, Shree Business Class Building, TC College Rd, Pragati Nagar, Baramati, Maharashtra 413102</p>
             <p className="office-contact">
              Kiran Sable Patil:<a href="tel:+919665240518">+91 96652 40518</a><br />
              Yash Gulumkar:<a href="tel:+919834562220">+91 98345 62220</a><br />
              Office Contact:<a href="tel:+91 84858 73830">+91 84858 73830</a>
            </p>
          </article>
          <article className="office-card">
            <span className="office-card__tag">Branch Office</span>
            <h3>Pune</h3>
            <p>Gagan Avincia, Office No. 401, Fourth Floor, Kharadi, Pune</p>
            <p className="office-contact">
              Kiran Sable Patil:<a href="tel:+919665240518">+91 96652 40518</a><br />
              Yash Gulumkar:<a href="tel:+919834562220">+91 98345 62220</a>
            </p>
          </article>
          <article className="office-card">
            <span className="office-card__tag">Regional Office</span>
            <h3>Beed</h3>
            <p>Flat No. 103, Bhosle Complex, Near Ambika Chowk, Beed 431122</p>
            <p className="office-contact">Ashok Karad — <a href="tel:+919588483355">+91 95884 83355</a></p>
          </article>
          <article className="office-card">
            <span className="office-card__tag">Regional Office</span>
            <h3>Thane</h3>
            <p>Room no 405, shree ekveera apratment lokmanya nagar, pada no 2 yashodhan nagar, Thane west, thane 400606</p>
             <p className="office-contact">
              Kiran Sable Patil — <a href="tel:+919665240518">+91 96652 40518</a><br />
              Yash Gulumkar — <a href="tel:+919834562220">+91 98345 62220</a>
            </p>
          </article>
          <article className="office-card">
            <span className="office-card__tag">Sales</span>
            <h3>Sales Desk</h3>
            <p>Mr. Ravi Pawar</p>
            <p className="office-contact"><a href="tel:+919689490469">+91 96894 90469</a></p>
          </article>
          <article className="office-card">
            <span className="office-card__tag">Service</span>
            <h3>Service Desk</h3>
            <p>Mr. Manohar Gulumkar</p>
            <p className="office-contact"><a href="tel:+919561284615">+91 95612 84615</a></p>
          </article>
        </div>
      </div>
    </section>
  )
}
